...
public class Movement_BaseLifeform : MonoBehaviourPun
{
    [HideInInspector]
    public Rigidbody RB;
    ...
    public float normalRotSpeed = 5f;
    ...
    void NewNormalDetect()
    {
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            Debug.DrawRay(RB.transform.position - RB.transform.up * 0.75f + offset * checkDist, -RB.transform.up * 0.5f, Color.blue);
            if (Physics.Raycast(RB.transform.position - RB.transform.up * 0.75f + offset * checkDist, -RB.transform.up, out hit, 1, layerMask))
            {
            ...
            Debug.DrawRay(RB.transform.position, offset * 0.5f, Color.blue);
            if (Physics.Raycast(RB.transform.position, offset, out hit, 1, layerMask))
            {
                if (hit.transform.gameObject.tag == "Crawlable")
                {
                    vect += hit.normal;
        ...
        Debug.DrawRay(RB.transform.position - RB.transform.up * 0.75f, -RB.transform.up * 0.5f, Color.blue);
        if (Physics.Raycast(RB.transform.position, -RB.transform.up * 0.75f, out hit, 1, layerMask))
        {
            if (hit.transform.gameObject.tag == "Crawlable")
            {
                vect += hit.normal;
    ...
    void RotateToNewNormal()
    {
        ...
        Vector3 tempVec = Vector3.SmoothDamp(RB.transform.up, vect,  ref velocity, normalRotSpeed);
